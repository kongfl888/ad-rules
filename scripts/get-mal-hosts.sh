...
if [ ! -s /tmp/malhosts.txt -a -s $oldhtxt ]; then
    ...
    grep -E "\.(com|net|cn|cc)" /tmp/malhosts.txt 2>/dev/null | sort -u >  $htxt
fi
sed -i -r -e 's/#.*//' -e '/^\S*$/d' $htxt
cp -f $oldhtxt /tmp/malhosts_new.txt
sed -i -r -e 's/#.*//' -e '/^\S*$/d' /tmp/malhosts_new.txt

if [ "$(f_md5 /tmp/malhosts_new.txt)" == "$(f_md5 $htxt)" ];then
cp -f $oldhtxt $htxt
else
